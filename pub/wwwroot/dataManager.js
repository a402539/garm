(function () {
  ...
  */
  ...
  var Renderer = {
    renderPath: function renderPath(ctx, feature) {
      // ctx.beginPath();
  ...
  function getBoxTiles(_x, _x2) {
    return _getBoxTiles.apply(this, arguments);
  }

  function _getBoxTiles() {
    _getBoxTiles = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(signal, bbox) {
      var _bbox, xmin, ymin, xmax, ymax, response;

      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _bbox = _slicedToArray(bbox, 4), xmin = _bbox[0], ymin = _bbox[1], xmax = _bbox[2], ymax = _bbox[3];
              _context.next = 3;
              return fetch('/box', {
                signal: signal,
                method: 'POST',
  ...
  function getTilePromise(layerId, z, x, y) {
    return fetch("/tile/".concat(layerId, "/").concat(z, "/").concat(x, "/").concat(y)).then(function (res) {
      return res.blob();
    }).then(function (blob) {
      ...
      Object.keys(layers).forEach(function (k) {
        ...
        var _loop = function _loop(i) {
          ...
          var properties = vf.properties;
          var coordinates = vf.loadGeometry();
          var path1 = new Path2D();
          coordinates[0].forEach(function (p, i) {
            if (i) {
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              _context2.next = 5;
              return getBoxTiles(abortController.signal, bbox);

            case 5:
              layersArr = _context2.sent;
              layersArr.forEach(function (layerItem) {
                var layerId = layerItem.layerId,
                    tiles = layerItem.tiles;

                if (!layerId) {
                  return;
                }

                var layerData = visibleLayers[layerId];
                var promises = layerData.promises || {};
                tiles.forEach(function (_ref) {
                  var z = _ref.z,
                      x = _ref.x,
                      y = _ref.y;
                  var tKey = "".concat(x, ":").concat(y, ":").concat(z);
                ...
                var tm = Date.now();
                var cnt = 0; // Promise.all(Object.values(promises))
                // promise.then(tiles => {		
                // tiles.forEach(tile => {

                Object.values(promises).forEach(function (promise) {
                  promise.then(function (tile) {
                    ...
                    ctx.lineWidth = 1 / sc;
                    Object.keys(layers).forEach(function (k) {
                      var features = layers[k].features;
                    ...
                    console.log('tile tm:', x, y, z, Date.now() - tm); // bitmapToMain(layerId, canvas);
  ...
  var chkEvent = function chkEvent(ev) {
    switch (ev.type) {
      case 'mousemove':
        ...
        var _loop2 = function _loop2(layerId) {
          var layerData = visibleLayers[layerId];
          var canvas = layerData.canvas;
          var ctx = canvas.getContext('2d');
          ...
          if (p[3]) {
            getItemsByPoint(layerData, ev.mapMousePos, ev.zoom).then(function (items) {
              self.postMessage({
                cmd: 'mouseover',
                layerId: layerId,
                items: items
        ...
        for (var layerId in visibleLayers) {
          _loop2(layerId);
        }
  ...
  var bitmapToMain = function bitmapToMain(layerId, canvas) {
